<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <!-- Owned Projects Section (Admin/Manager) -->
            @if (auth()->user()->role !== 'viewer')
                <div class="mb-12">
                    <div class="mb-6 flex justify-between items-center">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-900">My Projects</h2>
                            <p class="text-sm text-gray-600 mt-1">Projects you own and manage</p>
                        </div>
                        <div class="flex gap-3">
                            <a href="{{ route('projects.export.excel') }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:bg-green-700 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    ...
                    @if ($ownedProjects->isEmpty())
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                            <p class="text-gray-500">You haven't created any projects yet. 
                                @can('create', App\Models\Project::class)
                                    <a href="{{ route('projects.create') }}" class="text-indigo-600 hover:text-indigo-900 font-medium">Create your first project</a> to get started.
                                @endcan
                            </p>
                        </div>
                    @else
                                        ...
                                        <div class="flex justify-between items-start mb-4">
                                            <div class="flex-1">
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <span class="inline-flex items-center px-2 py-1 rounded text-xs font-semibold bg-blue-100 text-blue-800">Owned</span>
                                                </div>
            ...
            @if (auth()->user()->role === 'manager' || auth()->user()->role === 'admin')
                @if (!$assignedProjects->isEmpty())
                    <div>
                        <div class="mb-6">
                            <h2 class="text-2xl font-bold text-gray-900">Assigned to Me</h2>
                            <p class="text-sm text-gray-600 mt-1">Projects where you're a team member</p>
                        </div>
                                    ...
                                    <div class="p-6">
                                        <div class="flex justify-between items-start mb-4">
                                            <div class="flex-1">
                                                <div class="flex items-center gap-2">
                                                    <h3 class="text-lg font-bold text-gray-900">{{ $project->title }}</h3>
                                                    <span class="inline-flex items-center px-2 py-1 rounded text-xs font-semibold bg-green-100 text-green-800">Assigned</span>
                                                </div>
                                                <p class="text-sm text-gray-600 mt-1">{{ $project->description ?? 'No description' }}</p>
                                            </div>
                                        </div>
            ...
            @if (auth()->user()->role === 'admin')
                @if (!$otherProjects->isEmpty())
                    <div class="mb-12">
                        <div class="mb-6">
                            <h2 class="text-2xl font-bold text-gray-900">Other Projects</h2>
                            <p class="text-sm text-gray-600 mt-1">Projects you don't own or participate in</p>
                        </div>
                                    ...
                                    <div class="p-6">
                                        <div class="flex justify-between items-start mb-4">
                                            <div class="flex-1">
                                                <div class="flex items-center gap-2">
                                                    <h3 class="text-lg font-bold text-gray-900">{{ $project->title }}</h3>
                                                    <span class="inline-flex items-center px-2 py-1 rounded text-xs font-semibold bg-gray-100 text-gray-800">Other</span>
                                                </div>
                                                <p class="text-sm text-gray-600 mt-1">{{ $project->description ?? 'No description' }}</p>
                                            </div>
                                        </div>
            ...
            <!-- Viewers: Only Assigned Projects -->
            @if (auth()->user()->role === 'viewer')
                <div class="mb-6">
                    <h2 class="text-2xl font-bold text-gray-900">Projects Assigned to You</h2>
                    <p class="text-sm text-gray-600 mt-1">View and update milestones you're assigned to</p>
                </div>

                @if ($assignedProjects->isEmpty())
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <p class="text-gray-500">You haven't been assigned to any projects yet. You will see projects here when they are assigned to you.</p>
                    </div>
                @else
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($assignedProjects as $project)
